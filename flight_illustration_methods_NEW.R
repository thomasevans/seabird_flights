...
par(ps = 14, cex = 1.5, cex.lab = 2)
  ...
  svg(paste("map_illustration_figure_", flight_id, "_NEW.svg", sep = ""),
      width = 3, height = 9, family = "serif")
# ?cairo_ps
  # Plot base map
  par(mfrow = c(1,1))
 ...
 dif.co <- max(c(range.x,range.y))
  ...
  line.col[points.sub$included_points == FALSE] <- "dark grey"
  ...
  outline.col <- rep("black", length(points.size))
  outline.col[points.sub$included_points == FALSE] <- "grey50"
  points(points.sub$longitude, points.sub$latitude, cex = points.size,
         col = outline.col, bg = points.col.alpha, pch = 21)
  # ?points
...
# No x-axis for A + B, but common range as for C, so can use common X-axis
  ...
  svg(paste("gps_plot_test_illustration_", flight_id, "_NEW.svg", sep = ""),
      width = 4, height = 9, family = "serif")
  # ?pdf
...
# - horizontal line to indicate median value during flight
  ...
  points(points.sub$altitude_callib~ points.sub$date_time,
         pch = 21,
         col = outline.col, bg = points.col.alpha,
         # bg = points.col,
         cex = 1.5)
...
# Altitude vs. wind-speed - for example track
  ...
  svg(paste("wind_shear_example_test_illustration_", flight_id, "_NEW.svg", sep = ""),
...
# Could use common notation, for simplicity etc.
    ...
    svg(paste("vector_decompose_illustration_", flight_id, "_new.svg", sep = ""),
...
#          col = "#7570b3")
    ...
    arrows(0, 0, points.sub$ecmwf_wind_10m_u_flt_ht[pid],
           ...
           length = 0.15,
           col = "#1b9e77")
    text(points.sub$ecmwf_wind_10m_u_flt_ht[pid] + sign(points.sub$ecmwf_wind_10m_u_flt_ht[pid])*0.8,
         points.sub$ecmwf_wind_10m_v_flt_ht[pid] + sign(points.sub$ecmwf_wind_10m_v_flt_ht[pid])*0.6 - 0.9,
         "Vw",
         col = "#1b9e77")
