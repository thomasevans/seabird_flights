...
# - With wind components relative to goal, not to heading
...
# 3. Vw-side and Vw-head (relative to goal - not heading) --------

# Make df in form for ggplot:
# - v  ~ species + type (Vwc or Vwa)
df_wind_comp <- melt(flight_alt_ok,
                 id.vars = "sp_name", measure.vars = c("goal_cross_wind_10m_alt_filter", "goal_head_wind_10m_alt_filter"))
...
ggsave("plot_winds_goal.svg", plot = plot_winds, width = 6, height = 4, units = "in")
...
# "goal_cross_wind_10m_alt_filter", "goal_head_wind_10m_alt_filter"
# flight_alt_ok$goal_head_wind_flt_ht_alt_filter

# Type of wind assistance
flight_alt_ok$head_wind_flt_ht_alt_filter_type <- sign(flight_alt_ok$goal_head_wind_flt_ht_alt_filter)
...
# head(flight_alt_ok$goal_head_wind_flt_ht_alt_filter)
# head(flight_alt_ok$head_wind_flt_ht_alt_filter_type)

# Wind assistance absolute value
flight_alt_ok$head_wind_flt_ht_alt_filter_abs<- abs(flight_alt_ok$goal_head_wind_flt_ht_alt_filter)
hist(flight_alt_ok$head_wind_flt_ht_alt_filter_abs)


# Type of wind side
flight_alt_ok$cross_wind_flt_ht_alt_filter_type <- sign(flight_alt_ok$goal_cross_wind_flt_ht_alt_filter)
...
# Wind assistance absolute value
flight_alt_ok$cross_wind_flt_ht_alt_filter_abs<- abs(flight_alt_ok$goal_cross_wind_flt_ht_alt_filter)
...
write.csv(models.gull.fit.df, file = "gull_mod_goal.csv")

# Comparing model 7 and 8 
anova(models.gull.stdz[[7]],models.gull.stdz[[8]])
...
# Type of wind assistance
flight_alt$head_wind_flt_ht_type <- sign(flight_alt$goal_head_wind_flt_ht)
...
# Wind assistance absolute value
flight_alt$head_wind_flt_ht_abs<- abs(flight_alt$goal_head_wind_flt_ht)
# hist(flight_alt$head_wind_flt_ht_abs)


# Type of wind side
flight_alt$cross_wind_flt_ht_type <- sign(flight_alt$goal_cross_wind_flt_ht)
...
# Wind assistance absolute value
flight_alt$cross_wind_flt_ht_abs<- abs(flight_alt$goal_cross_wind_flt_ht)
...
# Type of wind assistance
flight_10m$head_wind_10m_type <- sign(flight_10m$goal_head_wind_10m)
...
# Wind assistance absolute value
flight_10m$head_wind_10m_abs<- abs(flight_10m$goal_head_wind_10m)
hist(flight_10m$head_wind_10m_abs)

# Type of wind side
flight_10m$cross_wind_10m_type <- sign(flight_10m$goal_cross_wind_10m)
...
# Wind assistance absolute value
flight_10m$cross_wind_10m_abs<- abs(flight_10m$goal_cross_wind_10m)
...
write.csv(models.gull.fit.all, file = "gull_va_model_fit_table_goal.csv")
#
...
write.csv(models.murre.fit.df, file = "models.murre.fit.df.filtered.goal.csv")

# Compare model 8 and 4
anova(models.murre.stdz[[4]],
      models.murre.stdz[[8]])
...
anova(models.murre.flt_ht_no_filter.stdz[[4]],
      models.murre.flt_ht_no_filter.stdz[[8]])
...
# Compare model 7 and 8
anova(models.murre.10m_no_filter.stdz[[7]],models.murre.10m_no_filter.stdz[[8]])
...
# Type of wind assistance
# flight_1m$goal_head_wind_1m

# flight_1m$goal_head_wind_1m <- flight_1m$ecmwf_wind_1m_speed_1m


flight_1m$head_wind_1m_type <- sign(flight_1m$goal_head_wind_1m)
...
# Wind assistance absolute value
flight_1m$head_wind_1m_abs<- abs(flight_1m$goal_head_wind_1m)
hist(flight_1m$head_wind_1m_abs)

# Type of wind side
flight_1m$cross_wind_1m_type <- sign(flight_1m$goal_cross_wind_1m)
...
# Wind assistance absolute value
flight_1m$cross_wind_1m_abs<- abs(flight_1m$goal_cross_wind_1m)
...
# hist(flight_1m_murre$va_1m - flight_10m_murre$va_10m)
...
models.murre.1m_no_filter[[2]] <- glmer(va_1m ~
                                          ...
                                          + (1|ring_number),
                                        data = flight_1m_murre)
models.murre.1m_no_filter[[1]] <- glmer(va_1m ~
                                          # trunc_seg_dist_a_b_km +
                                          # cross_wind_1m_abs +
                                          # head_wind_1m_abs*head_wind_1m_type +
                                     1     + (1|ring_number),
...
# Data preparation
# Make km version of distance
flight_2m <- flight_10m
flight_2m$trunc_seg_dist_a_b_km <- flight_2m$trunc_seg_dist_a_b / 1000
hist(flight_2m$trunc_seg_dist_a_b_km)

# Type of wind assistance
# flight_2m$goal_head_wind_2m

# flight_2m$goal_head_wind_2m <- flight_2m$ecmwf_wind_2m_speed_2m


flight_2m$head_wind_2m_type <- sign(flight_2m$goal_head_wind_2m)
summary(as.factor(flight_2m$head_wind_2m_type))
flight_2m$head_wind_2m_type <- factor(
  flight_2m$head_wind_2m_type, levels = c(-1, 1),
  labels = c("head", "tail")
)
summary(flight_2m$head_wind_2m_type)

# Wind assistance absolute value
flight_2m$head_wind_2m_abs<- abs(flight_2m$goal_head_wind_2m)
hist(flight_2m$head_wind_2m_abs)

# Type of wind side
flight_2m$cross_wind_2m_type <- sign(flight_2m$goal_cross_wind_2m)
summary(as.factor(flight_2m$cross_wind_2m_type))
flight_2m$cross_wind_2m_type <- factor(
  flight_2m$cross_wind_2m_type, levels = c(-1, 1),
  labels = c("to left", "to right")
)
summary(flight_2m$cross_wind_2m_type)

# Wind assistance absolute value
flight_2m$cross_wind_2m_abs<- abs(flight_2m$goal_cross_wind_2m)
hist(flight_2m$cross_wind_2m_abs)

# Species subsets
flight_2m_gull <- flight_2m[flight_2m$species == "gull",]
flight_2m_murre <- flight_2m[flight_2m$species == "murre",]



# hist(flight_2m_murre$va_2m - flight_10m_murre$va_10m)

# Fit possible models (8):
models.murre.2m_no_filter <- list()

models.murre.2m_no_filter[[8]] <- glmer(va_2m ~
                                         trunc_seg_dist_a_b_km +
                                          cross_wind_2m_abs +
                                          head_wind_2m_abs*head_wind_2m_type +
                                          + (1|ring_number),
                                        data = flight_2m_murre)

models.murre.2m_no_filter[[7]] <- glmer(va_2m ~
                                          # trunc_seg_dist_a_b_km +
                                          cross_wind_2m_abs +
                                          head_wind_2m_abs*head_wind_2m_type +
                                          + (1|ring_number),
                                        data = flight_2m_murre)
models.murre.2m_no_filter[[6]] <- glmer(va_2m ~
                                          trunc_seg_dist_a_b_km +
                                          cross_wind_2m_abs +
                                          # head_wind_2m_abs*head_wind_2m_type +
                                          + (1|ring_number),
                                        data = flight_2m_murre)
models.murre.2m_no_filter[[5]] <- glmer(va_2m ~
                                          trunc_seg_dist_a_b_km +
                                          # cross_wind_2m_abs +
                                          head_wind_2m_abs*head_wind_2m_type +
                                          + (1|ring_number),
                                        data = flight_2m_murre)
models.murre.2m_no_filter[[4]] <- glmer(va_2m ~
                                          # trunc_seg_dist_a_b_km +
                                          # cross_wind_2m_abs +
                                          head_wind_2m_abs*head_wind_2m_type +
                                          + (1|ring_number),
                                        data = flight_2m_murre)
models.murre.2m_no_filter[[3]] <- glmer(va_2m ~
                                          # trunc_seg_dist_a_b_km +
                                          cross_wind_2m_abs +
                                          # head_wind_2m_abs*head_wind_2m_type +
                                          + (1|ring_number),
                                        data = flight_2m_murre)
models.murre.2m_no_filter[[2]] <- glmer(va_2m ~
                                          trunc_seg_dist_a_b_km +
                                          # cross_wind_2m_abs +
                                          # head_wind_2m_abs*head_wind_2m_type +
                                          + (1|ring_number),
                                        data = flight_2m_murre)
models.murre.2m_no_filter[[1]] <- glmer(va_2m ~
                                          # trunc_seg_dist_a_b_km +
                                          # cross_wind_2m_abs +
                                          # head_wind_2m_abs*head_wind_2m_type +
                                     1     + (1|ring_number),
...
#                                         data = flight_2m_murre)
# 
# models.murre.2m_no_filter[[1]] <- glmer(va_2m ~ 1 +
#                                           (1|ring_number),
#                                         data = flight_2m_murre)
# #

# Standardize models
models.murre.2m_no_filter.stdz <- list()
models.murre.2m_no_filter.stdz[[1]] <- models.murre.2m_no_filter[[1]]
for(i in 2:8){
  models.murre.2m_no_filter.stdz[[i]] <- standardize(models.murre.2m_no_filter[[i]], standardize.y=FALSE)
}

models.murre.2m_no_filter.aicc <- sapply(models.murre.2m_no_filter.stdz, AICc)
models.murre.2m_no_filter.aicc.dif <- models.murre.2m_no_filter.aicc-min(models.murre.2m_no_filter.aicc)
models.murre.2m_no_filter.r2m <- sapply(models.murre.2m_no_filter.stdz, r.squaredGLMM)
t(models.murre.2m_no_filter.r2m)

models.murre.2m_no_filter.fit.df <- cbind.data.frame(c(1:8),models.murre.2m_no_filter.aicc,
                                                     models.murre.2m_no_filter.aicc.dif,
                                                     t(models.murre.2m_no_filter.r2m))
names(models.murre.2m_no_filter.fit.df) <- c("mod", "AICc", "dAICc", "R2m", "R2c")
...
# Data preparation
# Make km version of distance
flight_5m <- flight_10m
flight_5m$trunc_seg_dist_a_b_km <- flight_5m$trunc_seg_dist_a_b / 1000
hist(flight_5m$trunc_seg_dist_a_b_km)

# Type of wind assistance
# flight_5m$goal_head_wind_5m

# flight_5m$goal_head_wind_5m <- flight_5m$ecmwf_wind_5m_speed_5m


flight_5m$head_wind_5m_type <- sign(flight_5m$goal_head_wind_5m)
summary(as.factor(flight_5m$head_wind_5m_type))
flight_5m$head_wind_5m_type <- factor(
  flight_5m$head_wind_5m_type, levels = c(-1, 1),
  labels = c("head", "tail")
)
summary(flight_5m$head_wind_5m_type)

# Wind assistance absolute value
flight_5m$head_wind_5m_abs<- abs(flight_5m$goal_head_wind_5m)
hist(flight_5m$head_wind_5m_abs)

# Type of wind side
flight_5m$cross_wind_5m_type <- sign(flight_5m$goal_cross_wind_5m)
summary(as.factor(flight_5m$cross_wind_5m_type))
flight_5m$cross_wind_5m_type <- factor(
  flight_5m$cross_wind_5m_type, levels = c(-1, 1),
  labels = c("to left", "to right")
)
summary(flight_5m$cross_wind_5m_type)

# Wind assistance absolute value
flight_5m$cross_wind_5m_abs<- abs(flight_5m$goal_cross_wind_5m)
hist(flight_5m$cross_wind_5m_abs)

# Species subsets
flight_5m_gull <- flight_5m[flight_5m$species == "gull",]
flight_5m_murre <- flight_5m[flight_5m$species == "murre",]



# hist(flight_5m_murre$va_5m - flight_10m_murre$va_10m)

# Fit possible models (8):
models.murre.5m_no_filter <- list()

models.murre.5m_no_filter[[8]] <- glmer(va_5m ~
                                          trunc_seg_dist_a_b_km +
                                          cross_wind_5m_abs +
                                          head_wind_5m_abs*head_wind_5m_type +
                                          + (1|ring_number),
                                        data = flight_5m_murre)
models.murre.5m_no_filter[[7]] <- glmer(va_5m ~
                                          # trunc_seg_dist_a_b_km +
                                          cross_wind_5m_abs +
                                          head_wind_5m_abs*head_wind_5m_type +
                                          + (1|ring_number),
                                        data = flight_5m_murre)
models.murre.5m_no_filter[[6]] <- glmer(va_5m ~
                                          trunc_seg_dist_a_b_km +
                                          cross_wind_5m_abs +
                                          # head_wind_5m_abs*head_wind_5m_type +
                                          + (1|ring_number),
                                        data = flight_5m_murre)
models.murre.5m_no_filter[[5]] <- glmer(va_5m ~
                                          trunc_seg_dist_a_b_km +
                                          # cross_wind_5m_abs +
                                          head_wind_5m_abs*head_wind_5m_type +
                                          + (1|ring_number),
                                        data = flight_5m_murre)
models.murre.5m_no_filter[[4]] <- glmer(va_5m ~
                                          # trunc_seg_dist_a_b_km +
                                          # cross_wind_5m_abs +
                                          head_wind_5m_abs*head_wind_5m_type +
                                          + (1|ring_number),
                                        data = flight_5m_murre)
models.murre.5m_no_filter[[3]] <- glmer(va_5m ~
                                          # trunc_seg_dist_a_b_km +
                                          cross_wind_5m_abs +
                                          # head_wind_5m_abs*head_wind_5m_type +
                                          + (1|ring_number),
                                        data = flight_5m_murre)
models.murre.5m_no_filter[[2]] <- glmer(va_5m ~
                                          trunc_seg_dist_a_b_km +
                                          # cross_wind_5m_abs +
                                          # head_wind_5m_abs*head_wind_5m_type +
                                          + (1|ring_number),
...
models.murre.5m_no_filter[[1]] <- glmer(va_5m ~
#                                           trunc_seg_dist_a_b_km +
#                                           cross_wind_5m_abs +
#                                           head_wind_5m_abs*head_wind_5m_type +
                                     1     + (1|ring_number),
...
#                                         data = flight_5m_murre)
# 
# models.murre.5m_no_filter[[1]] <- glmer(va_5m ~ 1 +
#                                           (1|ring_number),
#                                         data = flight_5m_murre)
# #

# Standardize models
models.murre.5m_no_filter.stdz <- list()
models.murre.5m_no_filter.stdz[[1]] <- models.murre.5m_no_filter[[1]]
for(i in 2:8){
  models.murre.5m_no_filter.stdz[[i]] <- standardize(models.murre.5m_no_filter[[i]], standardize.y=FALSE)
}

models.murre.5m_no_filter.aicc <- sapply(models.murre.5m_no_filter.stdz, AICc)
models.murre.5m_no_filter.aicc.dif <- models.murre.5m_no_filter.aicc-min(models.murre.5m_no_filter.aicc)
models.murre.5m_no_filter.r2m <- sapply(models.murre.5m_no_filter.stdz, r.squaredGLMM)
t(models.murre.5m_no_filter.r2m)

models.murre.5m_no_filter.fit.df <- cbind.data.frame(c(1:8),models.murre.5m_no_filter.aicc,
                                                     models.murre.5m_no_filter.aicc.dif,
                                                     t(models.murre.5m_no_filter.r2m))
names(models.murre.5m_no_filter.fit.df) <- c("mod", "AICc", "dAICc", "R2m", "R2c")
...
write.csv(models.murre.fit.all, file = "murre_va_model_fit_table_goal.csv")
#
...
murre_model_va <- models.murre[[4]]
lbbg_model_va <- models.gull.flt_ht_no_filter[[8]]
summary(lbbg_model_va)
# murre_model_va <- models.murre.5m_no_filter[[8]]
# murre_model_va <- models.murre.10m_no_filter[[8]]


lbbg_model_va <- models.gull.10m_no_filter[[8]]

murre_model_va <- models.murre.flt_ht_no_filter[[4]]

# models.murre.10m_no_filter[[8]]
# models.murre.2m_no_filter[[8]]
...
allModelFrame$Variable[6:9] <- allModelFrame$Variable[2:5]
...
zp1 <- zp1 + scale_y_continuous(breaks = seq(-6, 20, 1),
                                minor_breaks = seq(-6, 20, 0.5))
                                # ,
                                # limits = c(-4, 20))
# ?scale_y_continuous
...
ggsave(zp1, filename = "va_model_coef_fig_goal.svg", width = 6, height = 8,
       units = "in")
# ?ggsave
...
# Prediction plots ----------
# If resume from here ----
load("airspeed_analysis_GOAL.RData")



wind.side <- seq(0,15,.1)
...
lab.4 <- expression(atop(paste("|Vw"["c"],"|", sep = ""),"Cross-wind"))
...
p <- ggplot(gg,aes(x,y)) + 
  ...
  scale_fill_gradient2(low = muted("blue"), mid = "white",
                       high = muted("red"), midpoint = median(flight_alt_ok_gg$z, na.rm = TRUE), space = "Lab",
  ...
  geom_point(data = flight_alt_ok_gg,
             aes(fill = z), 
             shape=21, alpha=0.8,na.rm=T, size=1.8) +
...
p <- p + annotate("text",  x= -9,
                  ...
                  vjust = 1, hjust=0, size = 5)
p
ggsave("va_gull_prediction_goal4.svg", width = 6, height = 4, units = "in")
...
#                   vjust = 1, hjust=0, size = 5)
p
ggsave("Va_gull_predication2_log_GOAL_poster.pdf",
       scale = 0.4, width = 350, height = 350, units = "mm",
       dpi = 600)
# ?ggsave
...
murre.new.data.df <- cbind.data.frame(
  trunc_seg_dist_a_b_km = dist.median,
  cross_wind_10m_abs = abs(gg$y),
  cross_wind_10m_type = factor(
    sign(gg$y), levels = c(-1, 1),
    labels = c("to left", "to right")),
  head_wind_10m_abs = abs(gg$x),
  head_wind_10m_type = factor(
    sign(gg$x), levels = c(-1, 1),
    labels = c("head", "tail"))
)
...
p <- ggplot(gg,aes(x,y)) + 
  ...
  scale_fill_gradient2(low = muted("blue"), mid = "white",
                       high = muted("red"),
                       midpoint = median(flight_alt_ok_gg$z, na.rm = TRUE), space = "Lab",
                       na.value = "grey50", guide = "colourbar") +
  scale_x_continuous(expand=c(0,0))+
  ...
  geom_point(data = flight_alt_ok_gg,
             aes(fill = z), 
             shape=21, alpha=0.8,na.rm=T, size=3) +
  labs( x = expression("Vw"["s"]~~~~"Wind assitance ("~ms^{-1}~")"),
        y = expression(paste("|Vw"["c"],"|", sep = "")~~~~"Cross wind ("~ms^{-1}~")"),
        fill = expression("Va ("~ms^{-1}~")"),
        parse = TRUE) +
...
ggsave("va_murre_predication_goal2.svg", width = 6, height = 4, units = "in")
...
#                   vjust = 1, hjust=0, size = 5)
p
ggsave("Va_murre_predication2_GOAL_poster.pdf",
       scale = 0.4, width = 350, height = 350, units = "mm",
       dpi = 600)
# ?ggsave
...
str(x)
x[,2]
# [1] 11.70413 11.48595 11.04980 10.58815 10.57708 10.68146 10.99835 10.84219 11.07618 10.94113
# [11] 11.00763 11.06298 11.12828 11.05650 10.48532 10.25406

murre.new.data.df <- cbind.data.frame(
  trunc_seg_dist_a_b_km = dist.median,
  cross_wind_10m_abs = 0,
  cross_wind_10m_type = factor(
    c(-1,1,-1,1), levels = c(-1, 1),
    labels = c("to left", "to right")),
  head_wind_10m_abs = 0,
...
str(x)
x[,2]
# [1] 14.89787 13.96929 16.01708 13.04651 13.32695 13.04037 13.49781 14.67886 15.33522 14.27410
# [11] 11.49553 11.13253 11.89644 14.61107 14.77792 13.54308 14.63569 13.17930 12.49256 13.99551
# [21] 14.01750