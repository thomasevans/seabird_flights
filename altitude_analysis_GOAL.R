...
# Analysing for wind with respect to goal direction
...
flight_alt_ok$head_wind_10m_alt_filter_type <- sign(flight_alt_ok$goal_head_wind_10m_alt_filter)
...
flight_alt_ok$head_wind_10m_alt_filter_abs<- abs(flight_alt_ok$goal_head_wind_10m_alt_filter)
hist(flight_alt_ok$head_wind_10m_alt_filter_abs)



# Type of wind side
flight_alt_ok$cross_wind_10m_alt_filter_type <- sign(flight_alt_ok$goal_cross_wind_10m_alt_filter)
...
# Wind assistance absolute value
flight_alt_ok$cross_wind_10m_alt_filter_abs<- abs(flight_alt_ok$goal_cross_wind_10m_alt_filter)
...
models.gull[[1]] <- glmer(altitude_callib_extm_log ~
                            1+
                            (1|ring_number),
                          data = flight_alt_ok_gull)
#
...
# Type of wind assistance
flight_alt$head_wind_10m_type <- sign(flight_alt$goal_head_wind_10m)
...
# Wind assistance absolute value
flight_alt$head_wind_10m_abs<- abs(flight_alt$goal_head_wind_10m)
hist(flight_alt$head_wind_10m_abs)


# Type of wind side
flight_alt$cross_wind_10m_type <- sign(flight_alt$goal_cross_wind_10m)
...
# Wind assistance absolute value
flight_alt$cross_wind_10m_abs<- abs(flight_alt$goal_cross_wind_10m)
...
write.csv(models.gull.fit.all, file = "gull_alt_model_fit_table_goal_new.csv")
#
...
# write.csv(models.murre.fit.all, file = "murre_alt_model_fit_table_goal_new.csv")

write.csv(models.murre.fit.all, file = "murre_alt_model_fit_table_influential_points_removed_goal_new.csv")
#
...
anova(models.gull.flt_ht_no_filter[[7]], models.gull.flt_ht_no_filter[[8]])
...
zp1
ggsave(zp1, filename = "alt_model_coef_fig_gull_log_GOAL.svg", width = 12, height = 4,
       units = "in")
# ?ggsave
...
p
ggsave("Alt_gull_predication2_log_GOAL.svg", width = 6, height = 4, units = "in")
...
p <- ggplot(gg,aes(x,y)) + 
  ...
  geom_point(data = flight_alt_ok_gg[!is.na(flight_alt_ok_gg$z) &
             ...
             aes(fill = z), 
             shape = 21, alpha = 0.8, na.rm = T, size = 2) +
  labs( x = expression("Vw"["s"]~~~~"Wind assitance ("~ms^{-1}~")"),
        y = expression(paste("|Vw"["c"],"|", sep = "")~~~~"Cross wind ("~ms^{-1}~")"),
        fill = "Altitude (m)",
        parse = TRUE) +
