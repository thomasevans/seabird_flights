...
# Gulls -----
mods.gull <- list()
mods.gull[[5]] <- glmer(vg ~ goal_head_wind_10m +
                          I(goal_head_wind_10m^2) +
                          goal_cross_wind_10m
                              + (1|ring_number),
                            data = flights.gulls)

mods.gull[[6]] <- glmer(vg ~ goal_head_wind_10m +
                          # I(goal_head_wind_10m^2) +
                          goal_cross_wind_10m
                        + (1|ring_number),
                        data = flights.gulls)

mods.gull[[4]] <- glmer(vg ~
                          # goal_head_wind_10m +
                          # I(goal_head_wind_10m^2) +
                          goal_cross_wind_10m
                        + (1|ring_number),
...
models.gull.aicc <- sapply(mods.gull, AICc)

models.gull.aicc.dif <- models.gull.aicc-min(models.gull.aicc)


models.gull.r2m <- sapply(mods.gull, r.squaredGLMM)
t(models.gull.r2m)
...
write.csv(x, file = "groundspeed_aic_etc_gulls_goal.csv")


# Murres -----

mods.murre <- list()
mods.murre[[5]] <- glmer(vg ~ goal_head_wind_10m +
                          I(goal_head_wind_10m^2) +
                          goal_cross_wind_10m
                        + (1|ring_number),
                        data = flights.murres)

mods.murre[[6]] <- glmer(vg ~ goal_head_wind_10m +
                          # I(goal_head_wind_10m^2) +
                          goal_cross_wind_10m
                        + (1|ring_number),
                        data = flights.murres)

mods.murre[[4]] <- glmer(vg ~
                          # goal_head_wind_10m +
                          # I(goal_head_wind_10m^2) +
                          goal_cross_wind_10m
                        + (1|ring_number),
...
models.murre.aicc <- sapply(mods.murre, AICc)

models.murre.aicc.dif <- models.murre.aicc-min(models.murre.aicc)


models.murre.r2m <- sapply(mods.murre, r.squaredGLMM)
t(models.murre.r2m)
d_r2m <- t(models.murre.r2m)[,1] - max(t(models.murre.r2m)[,1])
x <- cbind.data.frame(models.murre.aicc, models.murre.aicc.dif, t(models.murre.r2m), d_r2m)
write.csv(x, file = "groundspeed_aic_etc_murres_goal.csv")
...
drop1(mods.gull[[5]], test="user", sumFun=KRSumFun)


lbbg_model_va_coef <- summary(mods.gull[[5]])$coef[, 1]
lbbg_model_va_ci <- confint(mods.gull[[5]], method="Wald")
lbbg_model_va_par_df <- cbind.data.frame(lbbg_model_va_coef,lbbg_model_va_ci[-c(1:2),])


drop1(mods.murre[[2]], test="user", sumFun=KRSumFun)


murre_model_va_coef <- summary(mods.murre[[2]])$coef[, 1]
murre_model_va_ci <- confint(mods.murre[[2]], method="Wald")
...
mod.gull <- mods.gull[[5]]
mod.murre <- mods.murre[[2]]

mod_gull_frame <- data.frame(Variable = rownames(summary(mod.gull)$coef)[-1],
                             Coefficient = summary(mod.gull)$coef[-1, 1],
                             CI_low = confint(mod.gull, method="Wald")[-c(1:3), 1],
                             CI_high = confint(mod.gull, method="Wald")[-c(1:3), 2],
                             modelName = "Lesser Black-backed Gulls")
mod_murre_frame <- data.frame(Variable = rownames(summary(mod.murre)$coef)[-1],
                              Coefficient = summary(mod.murre)$coef[-1, 1],
                              CI_low = confint(mod.murre, method="Wald")[-c(1:3), 1],
                              CI_high = confint(mod.murre, method="Wald")[-c(1:3), 2],
...
allModelFrame$Variable <-  factor(allModelFrame$Variable,
                                   levels(allModelFrame$Variable)[c(1,3,2)])
...
zp1 <- zp1 +
  annotate("text",  x= layer_scales(zp1)$x$range$range[3],
           ...
           vjust= -3, hjust= 1, size = 5)
# zp1 <- zp1 + theme(legend.justification=c(0,0), legend.position=c(0,0))
zp1

ggsave(zp1, filename = "vg_model_coef_fig_GOAL.svg", width = 6, height = 4.5,
       units = "in")
...
lbbg.new.data.df <- cbind.data.frame(
  goal_head_wind_10m = (gg$x),
  goal_cross_wind_10m = (gg$y),
  ring_number = (gg$z)
)


# mean
pred.vg_quad <- predict(mod.gull,
                   newdata = lbbg.new.data.df[gg$z == birds[1],],
                   re.form=NA)
...
fit.data <- cbind.data.frame(pred.vg_quad, gg$x, gg$y, gg$z)
names(fit.data) <- c("vg", "goal_head_wind_10m", "goal_cross_wind_10m", "ring_number" )
g_gulls <- ggplot(flights.gulls,aes(x = goal_head_wind_10m , y = vg)) + 
  geom_vline(xintercept = 0) +
  geom_line(data = fit.data, alpha = 0.8,lwd = 1.2,
            col = cols.new[2]) +
#   geom_line(aes(x = goal_head_wind_10m , y = vg_linear),
...
#             col = cols.new[2]) +
  scale_x_continuous(breaks = seq(-10, 10, 2),
                       ...
                       limits = c(-11 , 11)) +
  geom_point(shape=21,na.rm=T, size=2, alpha = 0.7, col = "grey10") +
...
# parse = TRUE) 
ggsave("vg_gull_prediction_GOAL.svg", width = 6, height = 4, units = "in")
...
# mean
pred.vg_quad <- predict(mod.murre,
                        newdata = lbbg.new.data.df[gg$z == birds[1],],
                        re.form=NA)
# plot(pred.vg_quad~gg$x[gg$z == birds[1]])
# 
# pred.vg_linear <- predict(mod.murre.6,
#                    newdata = lbbg.new.data.df[gg$z == birds[1],],
#                    re.form=NA)
# plot(pred.vg_linear~gg$x[gg$z == birds[1]])
...
fit.data <- cbind.data.frame(pred.vg_quad, gg$x, gg$y, gg$z)
names(fit.data) <- c("vg", "goal_head_wind_10m", "goal_cross_wind_10m", "ring_number" )
g_murres <- ggplot(flights.murres,aes(x = goal_head_wind_10m , y = vg)) + 
  geom_vline(xintercept = 0) +
  geom_line(data = fit.data, alpha = 0.8,lwd = 1.2,
            col = cols.new[1]) +
  #   geom_line(aes(x = goal_head_wind_10m , y = vg_linear),
...
g_murres + annotate("text",  x= -10,
                   ...
                   vjust= 2, hjust= 0, size = 5)
# parse = TRUE) 
ggsave("vg_murre_prediction_GOAL.svg", width = 6, height = 4, units = "in")
...
# names(fit.data) <- c("vg", "vg_linear", "goal_head_wind_10m", "goal_cross_wind_10m", "ring_number" )
# g_murres <- ggplot(flights.murres,aes(x = goal_head_wind_10m , y = vg)) + 
#   geom_vline(xintercept = 0) +
#   geom_line(data = fit.data, alpha = 0.8,lwd = 1.2,
#             lty = 2,
